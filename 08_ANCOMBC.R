...
count_tab <- as.matrix(read.table("./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_Files/Seq2_MB_Count_Tab_No_Conta.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
# List of comparisons
comparisons <- c("Gallery", "2_tracts", "3_tracts", "4_tracts")
output_dir <- "./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_Files/ANCOMBC"
...
for (comparison in comparisons) {
  ...
  filename <- paste(output_dir, "Ancom_BC_Aggregate_", comparison, "_Family.tsv", sep = "")
...
#sample_data(ps_agglomerate)$Tract <- relevel(sample_data(ps_agglomerate)$Tract, "3_tracts") 

### Run ANCOMBC 
## Set.seed ensure reproducibility in statistical analyses or simulations.
   #--> When you set the seed, it means that the sequence of random numbers 
        #generated by R will be the same every time you run your code.
#set.seed(123) 
...
#write.table(res_agglo, file="./Output_Files_Seq_2_Methode_bacteria/Files/Stats_Files/ANCOMBC/Ancom_BC_Agglomerate_Bloc.tsv", sep="\t", quote=F, col.names=NA)
...
res_aggre_Gallery$lfc
tab_lfcG = res_aggre_Gallery$lfc
col_name = c("Taxon", "Intercept", "2 tracts vs Galleries", "3 tracts vs Galleries", "4 tracts vs Galleries") #Change the name with yours
colnames(tab_lfcG) = col_name
tab_lfcG %>% 
  datatable(caption = "Log Fold Changes from the Primary Result") %>%
  formatRound(col_name[-1], digits = 2)

lfc_all <- full_join(tab_lfc2, tab_lfc3, by = "Taxon")
lfc_all2 <- full_join(lfc_all, tab_lfc4, by = "Taxon")
lfc_ultime <- full_join(lfc_all2, tab_lfcG, by = "Taxon")
write.table(lfc_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/lfc_tab_Bacteria.tsv", sep="\t", quote=F, 
...
tab_seG = res_aggre_Gallery$se
colnames(tab_seG) = col_name
tab_seG %>% 
  datatable(caption = "SEs from the Primary Result") %>%
  formatRound(col_name[-1], digits = 2)

se_all <- full_join(tab_se2, tab_se3, by = "Taxon")
se_all2 <- full_join(se_all, tab_se4, by = "Taxon")
se_ultime <- full_join(se_all2, tab_seG, by = "Taxon")
write.table(se_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/se_tab_Bacteria.tsv", sep="\t", quote=F, 
            col.names=NA) #Change the name !
...
tab_wG = res_aggre_Gallery$W
colnames(tab_wG) = col_name
tab_wG %>% 
  datatable(caption = "Test Statistics from the Primary Result") %>%
  formatRound(col_name[-1], digits = 2)

w_all <- full_join(tab_w2, tab_w3, by = "Taxon")
w_all2 <- full_join(w_all, tab_w4, by = "Taxon")
w_ultime <- full_join(w_all2, tab_wG, by = "Taxon")
write.table(w_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/w_tab_Bacteria.tsv", sep="\t", quote=F, 
            col.names=NA) #Change the name !

# D) P-value
# --> p-value < 0.05 generally accepted

tab_pG = res_aggre_Gallery$p_val
colnames(tab_pG) = col_name
tab_pG %>% 
  datatable(caption = "P-values from the Primary Result") %>%
  formatRound(col_name[-1], digits = 2)

p_all <- full_join(tab_p2, tab_p3, by = "Taxon")
p_all2 <- full_join(p_all, tab_p4, by = "Taxon")
p_ultime <- full_join(p_all2, tab_pG, by = "Taxon")
write.table(p_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/pvalue_tab_Bacteria.tsv", sep="\t", quote=F, 
            col.names=NA) #Change the name !
...
tab_qG = res_aggre_Gallery$q
colnames(tab_qG) = col_name
tab_qG %>% 
  datatable(caption = "Adjusted p-values from the Primary Result") %>%
  formatRound(col_name[-1], digits = 2)

q_all <- full_join(tab_q2, tab_q3, by = "Taxon")
q_all2 <- full_join(q_all, tab_q4, by = "Taxon")
q_ultime <- full_join(q_all2, tab_qG, by = "Taxon")
write.table(q_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/adjusted_pvalue_tab_Bacteria.tsv", sep="\t", quote=F, 
            col.names=NA) #Change the name !
...
tab_diffG = res_aggre_Gallery$diff_abn
colnames(tab_diffG) = col_name
tab_diffG %>% 
  datatable(caption = "Differentially Abundant Taxa from the Primary Result")

diff_all <- full_join(tab_diff2, tab_diff3, by = "Taxon")
diff_all2 <- full_join(diff_all, tab_diff4, by = "Taxon")
diff_ultime <- full_join(diff_all2, tab_diffG, by = "Taxon")
write.table(diff_ultime, file="./Output_Files_Seq_2_Methode_Bacteria/Files/Stats_files/ANCOMBC/diff_abundance_tab_Bacteria.tsv", sep="\t", quote=F, 
            col.names=NA) #Change the name !
...
for (comparison in comparisons) {
  ...
  filename_lfc <- paste(output_dir, "LogFoldChange_", comparison, "_Family.tsv", sep = "")
  ...
  filename_se <- paste(output_dir, "StandardError_", comparison, "_Family.tsv", sep = "")
  ...
  filename_q <- paste(output_dir, "QValues_", comparison, "_Family.tsv", sep = "")
...
prout <- full_join(df_fig_tract2vs3, df_fig_tract3vs4, by = "taxon_id")
prout2 <- full_join(prout,df_fig_tract4vs2, by = "taxon_id")

proutG <- full_join(df_fig_tractGvs2,df_fig_tractGvs3,df_fig_tractGvs4, by = "taxon_id")
...
bubbleplot_all <- ggplot(proutultime, aes(x = taxon_id,)) +
                          geom_point(aes(y = "2 tracts vs 3 tracts", size = abs(Tract2_tracts), color = direct.x.x), alpha = 0.7, position = position_dodge(width = 0.5)) + # Add transparency for overlap
                          geom_point(aes(y = "3 tracts vs 4 tracts", size = abs(Tract3_tracts), color = direct.y.x), alpha = 0.7, position = position_dodge(width = 0.01)) +
                          ...
                          geom_point(aes(y = "Gallery vs 3 tracts", size = abs(TractGallery.y), color = direct.y.y), alpha = 0.7, position = position_dodge(width = 0.01)) +
                          geom_point(aes(y = "Gallery vs 4 tracts", size = abs(TractGallery), color = direct.y.y.y), alpha = 0.7, position = position_dodge(width = 0.01)) +
                          coord_flip() + 
                          scale_size_continuous(range = c(0.1, 6), breaks = c(1,3,5,7)) + # Adjust the size scale to your data
...
ggsave("C:/Users/apoll/OneDrive/Documents/Thèse/14_Articles/1_Methode/Figures/3_ANCOMBC/ANCOMBC_BubblePlot_all_family.jpeg", plot = bubbleplot_all, width = 12, height = 6, units = "in", dpi = 300)
ggsave("C:/Users/apoll/OneDrive/Documents/Thèse/14_Articles/1_Methode/Figures/3_ANCOMBC/ANCOMBC_BubblePlot_all_family.pdf", plot = bubbleplot_all, width = 12, height = 6, units = "in", dpi = 300)


#To add the SD, but in my case, it was not significant enought to be nicely visualise
#geom_point(aes(y = "2 tracts vs 3 tracts", size = abs(Tract2_tracts) + Tract2_tractsSE, color = direct.x.x), alpha = 0.20, shape=21) + geom_point(aes(y = "2 tracts vs 3 tracts", size = abs(Tract2_tracts) - Tract2_tractsSE, color = direct.x.x), alpha = 0.20, shape=21) + # Add transparency for overlap# Add transparency for overlap
  #geom_point(aes(y = "3 tracts vs 4 tracts", size = abs(Tract3_tracts) + Tract3_tractsSE, color = direct.y.x), alpha = 0.20, shape=21) + geom_point(aes(y = "3 tracts vs 4 tracts", size = abs(Tract3_tracts) - Tract3_tractsSE, color = direct.y.x), alpha = 0.20, shape=21) +
  #geom_point(aes(y = "4 tracts vs 2 tracts", size = abs(Tract4_tracts) + Tract4_tractsSE, color = direct.x.x.x),alpha = 0.20, shape=21) + geom_point(aes(y = "4 tracts vs 2 tracts", size = abs(Tract4_tracts) - Tract4_tractsSE, color = direct.x.x.x),alpha = 0.20, shape=21) + # Add transparency for overlap
  #geom_point(aes(y = "Gallery vs 2 tracts", size = abs(TractGallery.x) + TractGallerySE.x, color = direct.x.y), alpha = 0.20, shape=21) + geom_point(aes(y = "Gallery vs 2 tracts", size = abs(TractGallery.x) - TractGallerySE.x, color = direct.x.y), alpha = 0.20, shape=21) +
  #geom_point(aes(y = "Gallery vs 3 tracts", size = abs(TractGallery.y) + TractGallerySE.y, color = direct.y.y), alpha = 0.20, shape=21) + geom_point(aes(y = "Gallery vs 3 tracts", size = abs(TractGallery.y) - TractGallerySE.y, color = direct.y.y), alpha = 0.20, shape=21) +
  #geom_point(aes(y = "Gallery vs 4 tracts", size = abs(TractGallery) + TractGallerySE, color = direct.y.y.y), alpha = 1, shape=21) + geom_point(aes(y = "Gallery vs 4 tracts", size = abs(TractGallery) - TractGallerySE, color = direct.y.y.y), alpha = 0.20, shape=21) +
