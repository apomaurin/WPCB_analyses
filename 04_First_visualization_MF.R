...
count_tab <- as.matrix(read.table("./Output_Files_Seq_2_Methode_Fungi/Files/Stats_Files/Seq2_MF_Count_Tab.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
sample_info <- as.data.frame(read.table("./Output_Files_Seq_2_Methode_Fungi/Files/Stats_Files/Seq2_MF_Sample_info.tsv", header=T, row.names=1, check.names=F, sep="\t"))
...
### There isn't any clear differences between gallery and insect with the fungi. 
    # I'll remove the same sample as I did for the bacteria. 

### I will remove them from, but first, I'd like to do a hierarchical
...
### It's hard to say, but I'll delete the same sample as for the bacteria.
